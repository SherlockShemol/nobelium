...
export default function Header({ navBarTitle, fullWidth }: HeaderProps) {
  ...
  const resolveFavicon = (isDark: boolean | null, fallback: boolean): string => 
    !fallback && isDark ? '/favicon.dark.png' : '/favicon.png'
  ...
  function handleClickHeader(ev: MouseEvent<HTMLDivElement>) {
    const target = ev.target as Node
    if (navRef.current !== target && titleRef.current !== target) return
        ...
        <div className="flex items-center min-w-0 gap-1.5">
          <Link href="/" aria-label={BLOG.title} className="flex-shrink-0">
            <Image
              ...
              height={24}
              alt={BLOG.title}
              onError={() => setFavicon('/favicon.png')}
            />
          </Link>
          <HeaderName
            ref={titleRef}
...
interface HeaderNameProps {
  ...
  postTitle?: string
  onClick: (ev: MouseEvent<HTMLParagraphElement>) => void
}
