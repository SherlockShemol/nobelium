...
interface TailwindConfig {
  theme?: {
    extend?: {
      colors?: {
        day?: { DEFAULT?: string }
        night?: { DEFAULT?: string }
      }
    }
  }
}

const tailwindConfig = tailwind as unknown as TailwindConfig
...
class MyDocument extends Document {
  ...
  render() {
    ...
    const initialColorScheme = appearanceMap[config.appearance] || ''
        ...
        <Head>
          ...
          {`
            .color-scheme-unset, .color-scheme-unset body {
              background-color: ${tailwindConfig.theme?.extend?.colors?.day?.DEFAULT || config.lightBackground} !important;
            }
            @media (prefers-color-scheme: dark) {
              .color-scheme-unset, .color-scheme-unset body {
                background-color: ${tailwindConfig.theme?.extend?.colors?.night?.DEFAULT || config.darkBackground} !important;
              }
            }
          `}
          </style>
